...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Kelola perusahaan tempat pelaksanaan magang.</p>
        </div>
        <x-button href="{{ route('admin.perusahaan.create') }}">Tambah Perusahaan</x-button>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.perusahaan.index') }}" class="flex flex-col gap-3 lg:flex-row">
            <div class="min-w-0 flex-1"><label for="q" class="sr-only">Cari perusahaan</label><input id="q" name="q" value="{{ $search }}" type="search" placeholder="Cari nama, alamat, telepon, email, atau penanggung jawab..."
            ...
            @if($search)<a href="{{ route('admin.perusahaan.index') }}" class="rounded-lg border border-slate-300 px-4 py-2.5 text-center text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>@endif
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead><tr>
                    <th class="px-5 py-3 text-left"><x-table-sort column="nama_instansi" label="Perusahaan" /></th>
                ...
                @forelse($instansi as $item)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="whitespace-nowrap px-5 py-4"><div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.perusahaan.destroy', $item) }}" onsubmit="return confirm('Hapus data perusahaan ini?')">@csrf @method('DELETE')<button type="submit" class="rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50">Hapus</button></form>
                        </div></td>
                    </tr>
                @empty<tr><td colspan="5" class="px-5 py-12 text-center text-sm text-slate-500">Belum ada data perusahaan.</td></tr>@endforelse
                </tbody>
            </table>
        </div>
